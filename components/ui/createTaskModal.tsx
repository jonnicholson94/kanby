...
export default function CreateTaskModal() {
    ...
    return (
        <div className={`${!isCreateTaskOpen && "hidden"}`}>
            <div className={`h-screen w-screen fixed top-0 left-0 bg-overlay opacity-70`} onClick={hideModal}></div>

            <form className="h-auto xs:w-[95%] md:w-[50%] bg-primaryBg rounded-md border border-border fixed top-[20%] xs:left-[2.5%] md:left-[25%] p-[30px] flex items-center justify-center flex-col" onSubmit={handleSubmit}>
                ...
                <div className="h-auto w-full xs:flex-col lg:flex-row flex items-center justify-center">
                    <div className="h-auto xs:w-full lg:flex-grow flex items-center justify-start">
                        <SelectPopup current={status} options={statusOptions} handleChange={handleStatusUpdate}  />
                        <SelectPopup current={category} options={categoryOptions} handleChange={handleCategoryUpdate} />
                    </div>
                    <div className="h-auto xs:w-full lg:flex-grow flex items-center justify-end xs:mt-[20px] lg:mt-[0px]">
                        <button className={`h-[35px] w-[80px] border border-border rounded-sm text-[12px] mr-[10px] ${loading && "opacity-30"}`} disabled={loading} onClick={hideModal} type="button">Cancel</button>
                        <button className={`h-[35px] w-[80px] bg-primaryCta text-ctaText rounded-sm font-bold text-[12px] flex items-center justify-center`} disabled={loading} type="submit">
                            { loading ? <Loader /> : "Save" }
                        </button>
                    </div>
                </div>
